...
function loadIllusionsFromFallback() {
  // Inline data — used when fetch() fails (file:// protocol)
  if (typeof window.__illusionsData !== 'undefined') {
    return window.__illusionsData;
  }
  return [];
}
    ...
    btn.addEventListener('click', () => {
      ...
      if (container) {
        fetch('data/illusions.json')
          .then(r => r.json())
          .then(data => renderIllusions(container, data));
      }
    });
  });
}
